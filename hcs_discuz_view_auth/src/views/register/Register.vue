<template>
  <div class="register">
    <div class="main-content">
      <el-row>
        <el-col :xs="1" :sm="4" :md="6" :lg="7" :xl="8">
          <div style="background-color: #eee"></div>
        </el-col>
        <el-col :xs="22" :sm="16" :md="12" :lg="10" :xl="8">
          <div class="register-form">
            <h2>注 册</h2>
            <el-form
              :rules="rules"
              :model="userInfo"
              
              >
              <el-form-item prop="email">
                <el-input placeholder="请输入邮箱" 
                  style="font-size: 15px;" 
                  v-model="userInfo.email" 
                  size="large" >
                  <template #prefix>
                    <el-icon>
                      <Message/>
                    </el-icon>
                  </template>
                  
                </el-input>
              </el-form-item>
              <el-form-item prop="code">
                <el-input placeholder="请输入邮箱验证码" 
                  style="font-size: 15px;" 
                  v-model="code" 
                  size="large" >
                  <template #prefix>
                    <el-icon>
                      <Message/>
                    </el-icon>
                  </template>
                  <template #append>
                    <el-button size="large" @click="handSendCode">发送验证码</el-button>
                  </template>
                </el-input>
              </el-form-item>
              <el-form-item prop="username">
                <el-input placeholder="设置你的用户名" 
                  style="font-size: 15px" 
                  v-model="userInfo.username" 
                  size="large" >
                  <template #prefix>
                    <el-icon>
                      <User/>
                    </el-icon>
                  </template>
                </el-input>
              </el-form-item>
              <el-form-item prop="password">
                <el-input placeholder="设置你的密码" 
                  style="font-size: 15px" 
                  type="password" show-password 
                  v-model="userInfo.password" 
                  size="large" >
                  <template #prefix>
                    <el-icon>
                      <Lock/>
                    </el-icon>
                  </template>
                </el-input>
              </el-form-item>
              <el-form-item style="padding-left: 20px; padding-right: 20px">
                <el-button type="primary"
                  round size="large" 
                  @click="handleRegister"
                  style="width: 100%; font-size: 15px">
                  注 册
                </el-button>
              </el-form-item>
            </el-form>
            <router-link to="/">已有账号？去登录</router-link>
          </div>
        </el-col>
        <el-col :xs="1" :sm="4" :md="6" :lg="7" :xl="8">
          <div style="background-color: #eee"></div>
        </el-col>
      </el-row>
    </div>
  </div>
</template>

<script>
import { User,Lock,Message } from '@element-plus/icons-vue'
import { ElMessageBox,ElMessage } from 'element-plus'
import config from '../../assets/js/config'
import Cookies from 'js-cookie'
import {userFind, regSendCode, userRegister} from '../../network/auth'

export default {
  components: {
    User,
    Lock,
    Message
  },
  data(){
    return {
      userInfo:{
        username: null,
        password: null,
        email: null
      },
      code: null,
      imgcodeid:null,
      codeimgurl:null,
      rules: null,

    }
  },
  mounted(){
    // 判断是否已经登录
    this.initUserInfo()
    this.getImgCode()
  },
  methods:{
    // 验证邮箱函数
    validateEmail() {
      // 判空
      if(this.userInfo.email === null){
        return false
      }
      // 去空格
      this.userInfo.email = this.userInfo.email.replace(/\s*/g,"");
      // 验证长度
      if(this.userInfo.email.length < 4 || this.userInfo.email.length > 40){
        return false
      }
      // 验证格式
      var re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      return re.test(this.userInfo.email);
    },
    handSendCode(){
      if(!this.validateEmail()){
        alert("邮箱格式有误！")
        return
      }
      ElMessageBox.prompt(
      '<img src='+ this.codeimgurl+'>',
      '请输入验证码',{
        dangerouslyUseHTMLString: true,
        confirmButtonText: 'OK',
        cancelButtonText: 'Cancel',
      }).then(() => {
          regSendCode(this.userInfo.email).then(res=>{
            if(res.data.success===false){
              ElMessage.error(res.data.message)
            }else{
              ElMessage.success("验证码已发送！")
            }
          })
        }).catch(() => {
          ElMessage({
            type: 'info',
            message: '取消发送！',
          })
        })
    },
    handleRegister(){
      // 邮箱验证
      if(!this.validateEmail()){
        alert("邮箱格式有误！")
        return
      }
      // 判空
      if(this.code === null){
        alert("邮箱验证码不能为空！")
        return
      }
      if(this.userInfo.username === null || this.userInfo.password === null){
        alert("用户名或密码不能为空！")
        return
      }
      //去空格
      this.code = this.code.replace(/\s*/g,"");
      this.userInfo.username = this.userInfo.username.replace(/\s*/g,"");
      this.userInfo.password = this.userInfo.password.replace(/\s*/g,"");
      // 验证长度
      if(this.code.length < 4 || this.code.length >8){
        alert("邮箱验证码长度为4-8！")
        return
      }
      if(this.userInfo.username.length < 4 || this.userInfo.username.length > 16){
        alert("用户名长度为4-16！")
        return
      }
      if(this.userInfo.password.length < 4 || this.userInfo.password.length > 16){
        alert("密码长度为4-16！")
        return
      }
      userRegister(this.userInfo, this.code).then(res=>{
        if(res.data.success===false){
          alert(res.data.message)
        }else{
          alert("注册成功！")
          this.$router.replace("/")
        }
      })
    },
    getImgCode(){
      this.imgcodeid = Math.round(Math.random()*899999+100000)
      this.codeimgurl = config.host+"/code/findimg?imgcodeid=" + this.imgcodeid;
    },
    initUserInfo(){
      if(Cookies.get("token")===null){
        return
      }
      userFind().then(res=>{
          if(res.data.success===true){
            //判断身份
            switch(res.data.datas.user.role){
              case 1:
              case 2: 
                window.location.href = config.userPage;
                break;
              case 3: 
                //去管理员端
                window.location.href = config.adminPage;
                break;
              default: 
                break;
            }
          }else{
            return
          }
      })
    },
  },
}
</script>

<style scoped>
.main-content {
  margin-top: 40px;
}
.register-form {
  background-color: rgba(255, 255, 255, 0.2);
  padding: 20px;
  border: 1px solid #eee;
  border-radius: 0px;
  box-shadow: 1px 1px 3px #eee;
}
a{
  text-decoration:none;
  color: #888
}
a:hover {
  color: #555
}
</style>